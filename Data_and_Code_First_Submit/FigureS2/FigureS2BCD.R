...
color_df <- read.csv(paste0("results/", amp, "_top10Order_color_meanValue.csv"))
raw_df <- read.csv(paste0("results/", amp, "_top10Order_barplot_data.csv"))
...
p_pro <- ggplot(fin_df, aes(x = Order, y = Value, fill = Taxonomy)) +
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = color_df$Color, name = "Protist") +
    theme_bw() +
    theme(
        plot.title = element_text(size = 7, color = "black", hjust = 0.5),
        plot.subtitle = element_text(size = 6, color = "black", hjust = 0.5),
...
name <- paste0(dir_name, "/FigureS2BCD")
width <- 9
height <- 16
