...
barplot_16s <- read.csv("data/16S_barplot_data_relative_sorted.csv")
barplot_its <- read.csv("data/ITS_barplot_data_relative_sorted.csv")
barplot_pro <- read.csv("data/Protist_barplot_data_relative_sorted.csv")
...
res_df <- raw_df[raw_df$Level == "Order",]
...
write.csv(color_df, paste0(dir_name, "/16S_top10Order_color_meanValue.csv"), row.names = F)
...
write.csv(melt_df, paste0(dir_name, "/16S_top10Order_barplot_data.csv"), row.names = F)
...
color_df <- data.frame(top_df[c("Taxonomy", "Mean")], Color = color_manual)
write.csv(color_df, paste0(dir_name, "/ITS_top10Order_color_meanValue.csv"), row.names = F)
...
melt_df <- melt_df[melt_df$Tree %in% tree_list,]
melt_df$Tree <- factor(melt_df$Tree, levels = fin_tree$tip.label)
write.csv(melt_df, paste0(dir_name, "/ITS_top10Order_barplot_data.csv"), row.names = F)
...
color_df <- data.frame(top_df[c("Taxonomy", "Mean")], Color = color_manual)
write.csv(color_df, paste0(dir_name, "/Protist_top10Order_color_meanValue.csv"), row.names = F)
...
melt_df <- melt_df[melt_df$Tree %in% tree_list,]
melt_df$Tree <- factor(melt_df$Tree, levels = fin_tree$tip.label)
write.csv(melt_df, paste0(dir_name, "/Protist_top10Order_barplot_data.csv"), row.names = F)
...
p_pro <- ggplot(melt_df, aes(x = Tree, y = Value, fill = Taxonomy)) +
    ...
    scale_y_continuous(labels = scales::percent) +
    scale_fill_manual(values = color_manual, name = "Protist") +
    coord_flip() +
    theme(plot.title = element_text(size = 7, color = "black", hjust = 0.5),
          plot.subtitle = element_text(size = 6, color = "black", hjust = 0.5),
          axis.title = element_text(size = 7, color = "black"),
...
name <- paste0(dir_name, "/FigureS2A")
width <- 10
height <- 16
