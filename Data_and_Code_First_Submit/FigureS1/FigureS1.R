...
data_df <- data.frame()
for (amp in amplicon) {
    tmp_df <- read.csv(paste0("data/",amp, "/shannon.csv"), row.names = 1)
...
data_df <- data.frame()
for (amp in amplicon) {
    tmp_df <- read.csv(paste0("data/",amp, "/observed_features.csv"), row.names = 1)
...
p_obs <- ggplot(data_df, aes(x = Depth, y = Value, color = Order), size = 3) + 
    geom_line() + 
    # geom_vline(aes(xintercept = 500000), linetype = "dashed", linewidth = 0.5, color = "#666")+
    labs(
        x = 'Sampling depth',
