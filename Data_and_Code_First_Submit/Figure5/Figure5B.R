...
lamda_file <- read.csv("./data/absolute_format_core_ASV_phylogenetic_signal_fungi.csv", row.names = 1) %>% 
...
base_tree <- ggtree(tree_grouped, layout = 'fan', right = TRUE, linewidth = 0.15, open.angle = 15) +  #显示有信号的，tree_grouped换成tree_grouped2就行，其它代码不变
  geom_aline(aes(color = Taxonomy), linetype = 'solid', size = 0.2, linewidth = 0.15, key_glyph = 'rect', show.legend = TRUE) +
  scale_color_manual(values = class_colors$Color, breaks = class_colors$Taxa) + 
  new_scale_color()
...
#########################tree all core ASV
ggsave(final_plot,
       filename = "./result/Figure5A_fungal_core_ASVs_lambda_rf.pdf", width = 12, height = 12,  dpi = 900, bg = "white")
...
ggsave(final_plot2,
       filename = "./result/Figure5A_fungal_core_ASVs_lambda_rf_nolegend.pdf",
       width = 115/2, height = 115/2, units = "mm", dpi = 900, bg = "white")

ggsave(final_plot2,
       filename = "./result/Figure5A_fungal_core_ASVs_lambda_rf_nolegend.tiff",
       width = 115/2, height = 115/2, units = "mm", dpi = 900, bg = "white")

ggsave(final_plot2,
       filename = "./result/Figure5A_fungal_core_ASVs_lambda_rf_nolegend.jpg",
       width = 115/2, height = 115/2, units = "mm", dpi = 900, bg = "white")
