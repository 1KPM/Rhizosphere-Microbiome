...
### Import Data ----------------------------------------------------------------
core_tax_all <- read.csv("data/All_core_ASV_taxonomy.csv", row.names = 1)
# ------------------------------------------------------------------------------
...
absolute_df <- read.csv(paste0("data/", amp, "_feature_absolute_abundance.csv"), row.names = 1)
tax_df <- read.csv(paste0("data/", amp, "_ASV_taxonomy.csv"))
...
name <- paste0(dir_name, "/FigureS3A")
...
data_df$Kingdom <- factor(data_df$Kingdom, levels = c("Bacteria", "Fungi", "Protist"))

p <- ggplot(data_df, aes(x = Order, y = Percent)) +
    geom_bar(
        stat = "identity",
        position = "stack",
        width = 0.68,
...
name <- paste0(dir_name, "/FigureS3B")
width <- 17
height <- 7
ggsave(paste0(name, ".png"), p, width = width, height = height, dpi = 600, units = "cm")
ggsave(paste0(name, ".pdf"), p, width = width, height = height, units = "cm")
...
name <- paste0(dir_name, "/FigureS3C")
width <- 17
height <- 7.4
ggsave(paste0(name, ".png"), p, width = width, height = height, dpi = 600, units = "cm")
ggsave(paste0(name, ".pdf"), p, width = width, height = height, units = "cm")
