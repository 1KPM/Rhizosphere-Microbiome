...
for (amp in amplicon) {
    top_order_df <- read.csv(paste0("data/", amp, "_top10Order_color_meanValue.csv"), header = T)
    top_order_df <- top_order_df[1:10,]
    
    absolute_df <- read.csv(paste0("data/", amp, "_barplot_data_absolute_sorted.csv"), header = T)
    absolute_df <- absolute_df %>%
        filter(Level == "Order", Taxonomy %in%  top_order_df$Taxonomy) %>%
        tibble::column_to_rownames(var = "Taxonomy") %>%
        select(-Mean, -Level)
    
    relative_df <- read.csv(paste0("data/", amp, "_barplot_data_relative_sorted.csv"), header = T)
    relative_df <- relative_df %>%
        filter(Level == "Order", Taxonomy %in%  top_order_df$Taxonomy) %>%
        tibble::column_to_rownames(var = "Taxonomy") %>%
        select(-Mean, -Level)
    top_order_df$Clade <- ifelse(amp == "16S", "Bacteira", ifelse(amp == "ITS", "Fungi", "Protist"))
...
write.csv(write_df, paste0('results/correlation.csv'), row.names = F)
...
pdf(paste0('results/Figure2_corrplot.pdf'), width = width / 2.54, height = height / 2.54)
corrplot(fin_matrix, diag = F, tl.col = 'black', cl.cex = 0.5, tl.cex = 0.5, cl.pos = 'b', cl.length = 5, cl.ratio = 0.2, tl.srt = 45)
dev.off()

png(paste0('results/Figure2_corrplot.png'), width = width, height = height, units = 'cm', res = 600)
corrplot(fin_matrix, diag = F, tl.col = 'black', cl.cex = 0.5, tl.cex = 0.5, cl.pos = 'b', cl.length = 5, cl.ratio = 0.2, tl.srt = 45)
dev.off()
...
width <- 12.7
height <- 3
name <- paste0(dir_name, "/Figure2_relative")
...
width <- 12.7
height <- 3
name <- paste0(dir_name, "/Figure2_absolute")
ggsave(paste0(name, ".png"), p, width = width, height = height, dpi = 600, units = "cm")
ggsave(paste0(name, ".pdf"), p, width = width, height = height, units = "cm")
ggsave(paste0(name, ".tiff"), p, width = width, height = height, dpi = 600, units = "cm", compression = "lzw")
