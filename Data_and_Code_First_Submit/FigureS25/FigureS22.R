...
plot_list <- list()
for (kin in kingdom){
  data <- read.csv(paste0("./data/",kin,"_qa_log_metastat_res_diff_KO_matrix(p0.05q0.2).csv"),row.names = 1)
...
combined_data <- map_dfr(microbes, function(microbe) {
  read.csv(paste0("./data/", microbe, "_ko_qa_log_tree_metastat_res_diff_KO_enrich(p0.05q0.2).csv")) %>%
    filter(FDR < 0.05) %>%
...
ggsave("./FigureS22.pdf", combined_plot, width = 11, height = 10, dpi = 900)
ggsave("./FigureS22.jpg", combined_plot, width = 11, height = 10, dpi = 900)
ggsave("./FigureS22.tiff", combined_plot, width = 11, height = 10, dpi = 900)