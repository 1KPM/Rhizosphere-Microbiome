...
lamda_file <- read.csv("../phylogenetic_signal/amplicon/core_asv/absolute_format_core_ASV_phylogenetic_signal_protist.csv", row.names = 1) %>% 
  rownames_to_column("FeatureID")


####统计韦恩图
order <- read.csv("../amplicons_randomforest/pro_qa_log_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(order) <- gsub("\\|.*", "", row.names(order))
order$ASVID <- row.names(order)
class <- read.csv("../amplicons_randomforest/pro_qa_log_Class_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(class) <- gsub("\\|.*", "", row.names(class))
class$ASVID <- row.names(class)
family <- read.csv("../amplicons_randomforest/pro_qa_log_Family_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(family) <- gsub("\\|.*", "", row.names(family))
family$ASVID <- row.names(family)
genus <- read.csv("../amplicons_randomforest/pro_qa_log_Genus_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
...
# write.csv(lambda_rf_file, file = "pro_core_asv_qa_log_lambda_indicators.csv")
...
win.metafile("protistan_class_venn.emf")
...
win.metafile("protistan_order_venn.emf")
...
win.metafile("protistan_family_venn.emf")
...
lambda_rf_file <- read.csv(file = "pro_core_asv_qa_log_lambda_indicators.csv")
table(subset(lambda_rf_file, FDR < 0.05 & Tree_Order != "NA")$Tree_Order)

lambda_rf_file$Taxa <- lambda_rf_file$Class

top_class_pro <- read.csv('../rawdata/amplicon/Protist_top10Class_color_meanValue.csv')

raw_df <- lambda_rf_file
raw_df$Taxa <- ifelse(raw_df$Taxa == "", "Unassigned", raw_df$Taxa)
raw_df$Taxa <- ifelse(raw_df$Taxa %in% top_class_pro$Taxa, raw_df$Taxa, 'Others')
...
color_df <- data.frame(
  Taxonomy = top_class_pro$Taxa,
  Color = top_class_pro$Color
)
...
name = './Class distribution co-evolved protist ASVs'
...
process_and_plot <- function(tax_level, level_order, file_suffix, facet_width_factor) {
  ...
  height <- 6.5
  base_name <- paste0('./Class count co-evolved protistan ASVs-', file_suffix)
