...
lamda_file <- read.csv("../phylogenetic_signal/amplicon/core_asv/absolute_format_core_ASV_phylogenetic_signal_bacteria.csv", row.names = 1) %>% 
  rownames_to_column("FeatureID")


####统计韦恩图
order <- read.csv("../amplicons_randomforest/bac_qa_log_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(order) <- gsub("\\|.*", "", row.names(order))
order$ASVID <- row.names(order)
class <- read.csv("../amplicons_randomforest/bac_qa_log_Class_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(class) <- gsub("\\|.*", "", row.names(class))
class$ASVID <- row.names(class)
family <- read.csv("../amplicons_randomforest/bac_qa_log_Family_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
row.names(family) <- gsub("\\|.*", "", row.names(family))
family$ASVID <- row.names(family)
genus <- read.csv("../amplicons_randomforest/bac_qa_log_Genus_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
...
# write.csv(lambda_rf_file, file = "bac_core_asv_qa_log_lambda_indicators.csv")
...
win.metafile("Bacterial_class_venn.emf")
...
win.metafile("Bacterial_order_venn.emf")
...
win.metafile("Bacterial_family_venn.emf")
...
lambda_rf_file <- read.csv(file = "bac_core_asv_qa_log_lambda_indicators.csv")
table(subset(lambda_rf_file, FDR < 0.05 & Tree_Order != "NA")$Tree_Order)

lambda_rf_file$Taxa <- lambda_rf_file$Class

top_class_bac <- read.csv('../rawdata/amplicon/16S_top10Class_color_meanValue.csv')
...
name = './Class distribution co-evolved Bacteria ASVs'
...
process_and_plot <- function(tax_level, level_order, file_suffix, facet_width_factor) {
  ...
  height <- 6.5
  base_name <- paste0('./Class count co-evolved Bacterial ASVs-', file_suffix)
