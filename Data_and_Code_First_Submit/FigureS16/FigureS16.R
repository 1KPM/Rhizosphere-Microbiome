...
hub_lists <- list()
for (o in order_list) {
  hub_info <- read.csv(paste0('./data/cross_network_hub_info_',o,'_absolute.csv'), row.names = 1)
...
##Figure4A2
all_hub_info <- read.csv(paste0('./data/cross_network_hub_info_absolute.csv'), row.names = 1)
...
##Figure4B
all_hub <- read.csv("./data/cross_network_hub_info_absolute.csv")
inter_hub <- intersect(common_hubs,all_hub$label)
KO_list <- substr(inter_hub, 2, nchar(inter_hub))

result_all<-read.csv("./data/level1_enrichment_list1.csv")

names(result_all)[names(result_all) == "Cluster"] <- "hub"
...
final_result <- read.csv(file="./data/ASV_interaction_distribution.csv")
...
##Figure4D
data <- read.csv("./data/all_enrichment_results.csv")
colnames(data)[ncol(data)] <- "Correlation"
colnames(data)[ncol(data)-1] <- "Clade"
...
p_bottom <- cowplot::plot_grid(
    ...
    ncol = 3, nrow = 1, rel_widths = c(1.2, 0.35, 2) 
)
p_bottom
...
width <- 17.5
height <- 20
ggsave(paste0("FigureS16", ".png"), p, width = width, height = height, dpi = 600, units = "cm")
ggsave(paste0("FigureS16", ".pdf"), p, width = width, height = height, units = "cm")
