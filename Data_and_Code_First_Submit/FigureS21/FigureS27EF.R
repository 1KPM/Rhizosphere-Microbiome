...
pro_lamda_file <- read.csv("../phylogenetic_signal/metagenomic/Protist_KO_phylogenetic_signal_qa(log).csv", row.names = 1) %>% 
  rownames_to_column("KOID")


####统计韦恩图
class <- read.csv("../metagenomics_randomforest/pro_ko_qa_log_Class_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
row.names(class) <- gsub("\\|.*", "", row.names(class))
class$KOID <- row.names(class)

order <- read.csv("../metagenomics_randomforest/pro_ko_qa_log_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
row.names(order) <- gsub("\\|.*", "", row.names(order))
order$KOID <- row.names(order)

family <- read.csv("../metagenomics_randomforest/pro_ko_qa_log_Family_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
...
names(pro_lamda_rf_file)[17:19] <- c("Tree_Class", "Tree_Order", "Tree_Family") 

# write.csv(pro_lamda_rf_file, file = "pro_KO_qa_log_lamda_indicators.csv")

pro_lamda_ko_list <- unique(pro_lamda_rf_file[pro_lamda_rf_file$FDR < 0.05,]$KOID)


class_venn <- Venn(list('Phylogenetically conserved KOs' = pro_lamda_ko_list, 
                        'Class indicators' = subset(class, P.adj < 0.05)$KOID))

# win.metafile("pro_KO_class_venn.emf")
...
win.metafile("pro_KO_order_venn.emf")
...
win.metafile("pro_KO_family_venn.emf")
...
pro_lamda_rf_file <- read.csv(file = "pro_KO_qa_log_lamda_indicators.csv")
pro_lamda_rf_file$Taxa <- pro_lamda_rf_file$level2
table(pro_lamda_rf_file$level2) # superpathway level

ko_superpathway_color <- read.csv('../rawdata/KO/superpathway_color.csv')


raw_df <- pro_lamda_rf_file[, c("KOID","lambda", "P", "FDR", "Taxa", "Tree_Class", "Tree_Order", "Tree_Family")] ###原文件是在KOID上合并的，在superpathway上有重复，如同一个KO多次在一个pathway上出现
raw_df <- unique.data.frame(raw_df) #去重复
raw_df <- raw_df[!is.na(raw_df$Taxa),] # 7920个

all_df <- data.frame(t(table(raw_df$Taxa)))
pro_lamda_df <- data.frame(t(table(raw_df[raw_df$FDR < 0.05, 'Taxa'])))
pro_rf_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]), 'Taxa'])))
co_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]) & raw_df$FDR < 0.05, 'Taxa'])))
pro_rf_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]), 'Taxa'])))
co_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]) & raw_df$FDR < 0.05, 'Taxa'])))
pro_rf_df_family <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Family"]), 'Taxa'])))
...
res_df <- res_df[,c(2,3,5,7,9,11,13)]
...
name = './Superpathway distribution protistan co-evolved KOs'
...
process_and_plot <- function(tax_level, level_order, file_suffix, facet_width_factor) {
  ...
  height <- 8.5
  base_name <- paste0('./Superpathway count protistan co-evolved KOs-', file_suffix)
