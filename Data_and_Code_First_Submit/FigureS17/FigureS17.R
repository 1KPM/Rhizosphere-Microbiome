...
##Fabales
node_mt2 <- read.csv("./data/FigureS17-Fabales_node_mt2.csv",row.names = 1)
edge_mt2 <- read.csv("./data/FigureS17-Fabales_edge_mt2.csv")
cross_mt2_p <- ggplot() + 
...
df<-read.csv('./data/FigureS17-Fabales_Cross_kingdom_network_hub_info.csv')
all_df <- df %>%
  mutate(
    Taxonomy = case_when(
...
raw_df<-read.csv('./data/FigureS17-Fabales_Cross_kingdom_network_hub_info.csv')
...
p1 <- cross_mt2_p + final_plot +plot_layout(widths = c(3,2))


###Rosales
node_mt2 <- read.csv("./data/FigureS17-Rosales_node_mt2.csv",row.names = 1)
edge_mt2 <- read.csv("./data/FigureS17-Rosales_edge_mt2.csv")
cross_mt2_p <- ggplot() + 
  geom_segment(aes(x = X1, y = Y1, xend = X2, yend = Y2, color = Correlation), 
               alpha = 0.01, linewidth = 0.1, data = edge_mt2) + 
  geom_point(aes(X1, X2, fill = Clade, size = Degree^0.5), pch = 21, data = node_mt2) +
...
df<-read.csv('./data/FigureS17-Rosales_Cross_kingdom_network_hub_info.csv')
all_df <- df %>%
  mutate(
    Taxonomy = case_when(
      startsWith(X, "b") ~ "Bacteria",
...
raw_df<-read.csv('./data/FigureS17-Rosales_Cross_kingdom_network_hub_info.csv')
color_manual <- colorRampPalette(brewer.pal(3, 'Accent'))(3)
names(raw_df)[2:4] <- c('Degree','Closeness','Betweenness')
...
p2 <- cross_mt2_p + final_plot +plot_layout(widths = c(3,2))


###Malpighiales
node_mt2 <- read.csv("./data/FigureS17-Malpighiales_node_mt2.csv",row.names = 1)
edge_mt2 <- read.csv("./data/FigureS17-Malpighiales_edge_mt2.csv")
cross_mt2_p <- ggplot() + 
  geom_segment(aes(x = X1, y = Y1, xend = X2, yend = Y2, color = Correlation), 
               alpha = 0.01, linewidth = 0.1, data = edge_mt2) + 
  geom_point(aes(X1, X2, fill = Clade, size = Degree^0.8), colour = "grey10",pch = 21, data = node_mt2) +
  scale_fill_manual(values = c("Bacteria"="#7FC97F","Fungi"="#BEAED4","Protist"="#FDC086"))  +
  scale_x_continuous(breaks = NULL) + scale_y_continuous(breaks = NULL) +
  theme(panel.background = element_blank()) +
  theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +
...
df<-read.csv('./data/FigureS17-Malpighiales_Cross_kingdom_network_hub_info.csv')
all_df <- df %>%
  mutate(
    Taxonomy = case_when(
      startsWith(X, "b") ~ "Bacteria",
...
raw_df<-read.csv('./data/FigureS17-Malpighiales_Cross_kingdom_network_hub_info.csv')
color_manual <- colorRampPalette(brewer.pal(3, 'Accent'))(3)
names(raw_df)[2:4] <- c('Degree','Closeness','Betweenness')
...
p3 <- cross_mt2_p + final_plot +plot_layout(widths = c(3,2))
...
width <- 30
height <- 60
ggsave('./FigureS17.pdf', all_fig, width = width, height = height, units = 'cm',dpi=600)
ggsave('./FigureS17.jpg', all_fig, width = width, height = height, units = 'cm',dpi=600)
ggsave('./FigureS17.tiff', all_fig, width = width, height = height, units = 'cm',dpi=600)