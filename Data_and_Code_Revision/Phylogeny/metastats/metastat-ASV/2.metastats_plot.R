...
for (kin in kingdom){
  tmp_res_all <- read.csv(paste0("./stats/",kin,"_qa_log_metastat_res_diff_ASV_core0.2.csv"),row.names = 1)
  ...
  # 创建存储每个目富集和排斥ASV列表的容器
  enriched_asv_lists <- setNames(replicate(length(order_list1), list(), simplify = FALSE), order_list1)
  depleted_asv_lists <- setNames(replicate(length(order_list1), list(), simplify = FALSE), order_list1)
  ...
  for (i in 1:(length(order_list1)-1)) {
    for (j in (i+1):length(order_list1)) {
      ...
      # 富集ASV
      up_asv <- subset_data %>% filter(Group == order_list1[i]) %>% pull(Taxa)
      down_asv <- subset_data %>% filter(Group == order_list1[j]) %>% pull(Taxa)
      
      comparison_matrix[i, j] <- length(up_asv)
      comparison_matrix[j, i] <- length(down_asv)
      
      # 添加到非冗余列表
      enriched_asv_lists[[order_list1[i]]] <- unique(c(enriched_asv_lists[[order_list1[i]]], up_asv))
      enriched_asv_lists[[order_list1[j]]] <- unique(c(enriched_asv_lists[[order_list1[j]]], down_asv))
      
      depleted_asv_lists[[order_list1[j]]] <- unique(c(depleted_asv_lists[[order_list1[j]]], up_asv))
      depleted_asv_lists[[order_list1[i]]] <- unique(c(depleted_asv_lists[[order_list1[i]]], down_asv))
    }
  }
  ...
  # 计算非冗余总数
  enriched_totals <- sapply(order_list, function(x) length(enriched_asv_lists[[x]]))
  depleted_totals <- sapply(order_list, function(x) length(depleted_asv_lists[[x]]))
  
  # 保存数据
  write.csv(comparison_matrix, file = paste0("./stats/",kin,"_qa_log_metastat_res_diff_asv_matrix(core0.2p0.05q0.2).csv"))
  write.csv(data.frame(Enriched = enriched_totals, Depleted = depleted_totals, row.names = order_list), 
            file = paste0("./stats/",kin,"_qa_log_metastat_res_diff_asv_totals(core0.2p0.05q0.2).csv"))
}
...
for (kin in kingdom) {
  # 读取数据
  data <- read.csv(paste0("./stats/",kin,"_qa_log_metastat_res_diff_asv_matrix(core0.2p0.05q0.2).csv"), row.names = 1)
  totals <- read.csv(paste0("./stats/",kin,"_qa_log_metastat_res_diff_asv_totals(core0.2p0.05q0.2).csv"), row.names = 1)
...
# 保存图形 - 显著减小宽度
ggsave("./plots/diff_asv_matrix_combined.png", final_plot, 
       width = 8, height = 24, dpi = 300, bg = "white", units = "cm", limitsize = FALSE)
ggsave("./plots/diff_asv_matrix_combined.pdf", final_plot, 
