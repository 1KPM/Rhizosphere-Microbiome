...
names(fun_ko_ra) <- gsub("RS.", "", names(fun_ko_ra))
fun_ko_qa <- read.csv(file = "../../../../kegg_abundance/absolute/Fungi_core0.2_quantitative.csv", row.names = 1, header = T)
names(fun_ko_qa) <- gsub("RS.", "", names(fun_ko_qa))


fun_lamda_file <- read.csv("../../../../12.phylogenetic_signal_λ/metagenomic/core_KO/Fungi_core0.2_qa_log_phylogenetic_signal.csv", row.names = 1) %>% 
  rownames_to_column("KOID")
table(fun_lamda_file$padj < 0.05) ###261个系统发图显著的KO

fun_rf_file <-  read.csv("../../../../13.random_forest/metagenome/fun_ko_core0.2_qa_log_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
# 该文件只包含显著性矫正P值小于0.2的KO
row.names(fun_rf_file) <- gsub("\\|.*", "", row.names(fun_rf_file))
fun_rf_file$KOID <- row.names(fun_rf_file) 
table(fun_rf_file$P.adj < 0.05) ###44个随机森林显著的KO 

# 数据合并
fun_group_file <- fun_lamda_file %>%
  left_join(fun_rf_file, by = "KOID") %>%
  left_join(core_annotation, by = "KOID") 

table(fun_group_file$Group)
table(fun_group_file$padj < 0.05)  ###261 ko
table(fun_group_file$P < 0.05)
table(fun_group_file$P.adj < 0.05) ###44 ko
...
#####################################################################################################
####统计韦恩图
fun_ko_list <- fun_group_file$KOID
fun_lamda_ko_list <- subset(fun_group_file, padj < 0.05)$KOID
fun_rf_ko_list <- subset(fun_group_file, P.adj < 0.05)$KOID
order_co_ko_list <- subset(fun_group_file, padj < 0.05 & P.adj < 0.05)$KOID #21


order_venn <- Venn(list('Phylogenetically conserved KOs' = fun_lamda_ko_list, 
                        'Order indicators' = fun_rf_ko_list))

win.metafile("./plots/fun_KO_order_venn.emf")
plot(order_venn, doWeights = TRUE, type="circles", 
     show = list(Faces = T, DarkMatter = FALSE))
dev.off()

###Class和Family水平
class <- read.csv("../../../../13.random_forest/metagenome/fun_ko_qa_log_Class_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
row.names(class) <- gsub("\\|.*", "", row.names(class))
class$KOID <- row.names(class)
family <- read.csv("../../../../13.random_forest/metagenome/fun_ko_qa_log_Family_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
row.names(family) <- gsub("\\|.*", "", row.names(family))
family$KOID <- row.names(family)

class_venn <- Venn(list('Phylogenetically conserved KOs' = fun_lamda_ko_list, 
                        'Class indicators' = subset(class, P.adj < 0.05)$KOID))

win.metafile("./plots/fun_KO_class_venn.emf")
plot(class_venn, doWeights = TRUE, type="circles",  
     show = list(Faces = T, DarkMatter = FALSE))
dev.off()

family_venn <- Venn(list('Phylogenetically conserved KOs' = fun_lamda_ko_list, 
                         'Family indicators' = subset(family, P.adj < 0.05)$KOID))

win.metafile("./plots/fun_KO_family_venn.emf")
...
fun_lamda_rf_file <- fun_lamda_file %>%
  left_join(core_annotation, by = "KOID") %>%
  left_join(core_pathway[,-8], by = "KOID") %>%
  left_join(class[class$P.adj < 0.05, c("KOID", "Group","P.adj")], by = "KOID") %>%
  left_join(fun_rf_file[fun_rf_file$P.adj < 0.05, c("KOID", "Group", "P.adj")], by = "KOID") %>%  #fun_rf_file就是order水平
  left_join(family[family$P.adj < 0.05, c("KOID", "Group","P.adj")], by = "KOID") 

names(fun_lamda_rf_file)[17:22] <- c("Tree_Class", "rf_FDR_Class", "Tree_Order", "rf_FDR_Order", "Tree_Family", "rf_FDR_Family") 

write.csv(fun_lamda_rf_file, file = "./stats/fun_KO_qa_log_lamda_indicators.csv")
# write.csv(unique(fun_lamda_rf_file[,-c(11:16)]), file = "./stats/TableS7E-fun_KO_qa_log_lamda_indicators.csv")

table(fun_lamda_rf_file[!is.na(fun_lamda_rf_file$Tree_Order) & fun_lamda_rf_file$padj < 0.05,]$Tree_Order)
#########

fun_lamda_rf_file <- read.csv(file = "./stats/fun_KO_qa_log_lamda_indicators.csv")
fun_lamda_rf_file$Superpathway <- fun_lamda_rf_file$level2
table(fun_lamda_rf_file$level2) # superpathway level

ko_superpathway_color <- read.csv('../../../../metadata/superpathway_color.csv')


raw_df <- fun_lamda_rf_file[, c("KOID","lambda", "P", "padj", "Superpathway", "Tree_Class", "Tree_Order", "Tree_Family")] 
###原文件是在KOID上合并的，在superpathway上有重复，如同一个KO多次在一个pathway上出现
raw_df <- unique.data.frame(raw_df) #去重复
raw_df <- raw_df[!is.na(raw_df$Superpathway),] # 4002个

all_df <- data.frame(t(table(raw_df$Superpathway)))
fun_lamda_df <- data.frame(t(table(raw_df[raw_df$padj < 0.05, 'Superpathway'])))
fun_rf_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]), 'Superpathway'])))
co_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]) & raw_df$padj < 0.05, 'Superpathway'])))
fun_rf_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]), 'Superpathway'])))
co_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]) & raw_df$padj < 0.05, 'Superpathway'])))
fun_rf_df_family <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Family"]), 'Superpathway'])))
co_df_family <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Family"]) & raw_df$padj < 0.05, 'Superpathway'])))

res_df <- all_df %>%
  left_join(fun_lamda_df, by = "Var2") %>%
  left_join(fun_rf_df_class, by = "Var2") %>%
  left_join(co_df_class, by = "Var2") %>%
  left_join(fun_rf_df_order, by = "Var2") %>%
  left_join(co_df_order, by = "Var2") %>%
  left_join(fun_rf_df_family, by = "Var2") %>%
...
name = './plots/Superpathway_distribution_bacterial_co-evolved KOs'
...
process_and_plot <- function(tax_level, level_order, file_suffix, facet_width_factor) {
  ...
  # 初始化结果数据框
  names(fun_lamda_df)[2] <- "Superpathway"
  res_df <- fun_lamda_df[-1]
  
  # 批量处理数据
  for (current_level in tax_levels) {
    # 创建指标数据（该分类下的所有Superpathway，无P值过滤）
    fun_rf_data <- data.frame(
    ...
    names(fun_rf_data)[2] <- paste(current_level, "indicators")
    ...
    # 合并数据
    res_df <- res_df %>% 
      left_join(fun_rf_data, by = "Superpathway") %>% 
  ...
  height <- 8.5
  base_name <- paste0('./plots/Superpathway_count_fungal_co-evolved-KOs-Number-', file_suffix)     # Number/Percentage
...
fun_lamda_rf_file <- read.csv(file = "./stats/fun_KO_qa_log_lamda_indicators.csv")
names(fun_lamda_rf_file)
fun_lamda_rf_enrich <- unique(fun_lamda_rf_file[,-c(1,11:17)]) ###5160行，5160个KO， 没错的

table(subset(fun_lamda_rf_enrich, padj < 0.05 & Tree_Order != "NA")$Tree_Order)

fun_core_ko_list <- fun_lamda_rf_enrich$KOID
fun_lamda_ko_list <- subset(fun_lamda_rf_enrich, padj < 0.05)$KOID
class_ko_list <- subset(fun_lamda_rf_enrich, Tree_Class != "NA")$KOID
class_co_ko_list <- subset(fun_lamda_rf_enrich, padj < 0.05 & Tree_Class != "NA")$KOID
order_ko_list <- subset(fun_lamda_rf_enrich, Tree_Order != "NA")$KOID
order_co_ko_list <- subset(fun_lamda_rf_enrich, padj < 0.05 & Tree_Order != "NA")$KOID
family_ko_list <- subset(fun_lamda_rf_enrich, Tree_Family != "NA")$KOID
family_co_ko_list <- subset(fun_lamda_rf_enrich, padj < 0.05 & Tree_Family != "NA")$KOID

table(fun_lamda_rf_enrich$Tree_Class)

#进行富集分析
fun_core_ko_result=enrichKEGG(fun_core_ko_list,
                          organism = "ko", #物种选择ko
...
fun_lamda_ko_result=enrichKEGG(fun_lamda_ko_list,
                            organism = "ko", #物种选择ko
                            pvalueCutoff = 0.05,  #p值cutoff
                            pAdjustMethod = "fdr", #fdr矫正p值
                            qvalueCutoff = 0.2   #q值cutoff
...
#保存富集分析结果
write.csv(file="./stats/KO_enrichment_all_fun_core_ko.csv", data.frame(fun_core_ko_result), row.names=F)
write.csv(file="./stats/KO_enrichment_all_fun_lamda_ko.csv", data.frame(fun_lamda_ko_result), row.names=F)
...
# 绘制气泡图
barplot(fun_core_ko_result)
barplot(fun_lamda_ko_result)
barplot(order_ko_result)
barplot(order_co_ko_result)

####整合图
inter_enriched <- rbind.data.frame(data.frame(fun_core_ko_result),
                                   data.frame(fun_lamda_ko_result),
...
inter_enriched$Group <- c(rep("All", dim(data.frame(fun_core_ko_result))[1]), 
                          rep("Phylogenetically conserved", dim(data.frame(fun_lamda_ko_result))[1]),
...
ggsave(inter_enriched_g,
       filename = "./plots/KO_enrichment_all_plant_class_order_family_bacterial_coevolved_ko.pdf",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
ggsave(inter_enriched_g,
       filename = "./plots/KO_enrichment_all_plant_class_order_family_bacterial_coevolved_ko.png",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
...
ggsave(inter_enriched_g2,
       filename = "./plots/KO_enrichment_fun_lamda_and_Phylogenetically_conserved_ko.pdf",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
ggsave(inter_enriched_g2,
       filename = "./plots/KO_enrichment_fun_lamda_and_Phylogenetically_conserved_ko.png",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
...
table(fun_lamda_rf_enrich$Tree_Order)

# 豆目没有检测到共进化的真菌KO，使用Asparagales
Asparagales_ko_list <- subset(fun_lamda_rf_enrich, Tree_Order == "Asparagales")$KOID
Asparagales_co_ko_list <- subset(fun_lamda_rf_enrich, Tree_Order == "Asparagales" & padj < 0.05)$KOID
#进行富集分析
...
#保存富集分析结果
write.csv(file="./stats/KO_enrichment_Asparagales_fun_coevolved_ko.csv", data.frame(Asparagales_co_ko_result), row.names=F)
write.csv(file="./stats/KO_enrichment_Asparagales_fun_rf_ko.csv", data.frame(Asparagales_ko_result), row.names=F)
...
for (order in orders) {
  ...
  # 提取当前目的基因列表
  order_ko_list <- subset(fun_lamda_rf_enrich, Tree_Order == order)$KOID
  order_co_ko_list <- subset(fun_lamda_rf_enrich, Tree_Order == order & padj < 0.05)$KOID
...
ggsave(order_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_orders_fun_coevolved_ko.pdf",
       width = 24, height = 12, units = "cm", dpi = 900, bg = "white")

ggsave(order_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_orders_fun_coevolved_ko.png",
       width = 24, height = 12, units = "cm", dpi = 900, bg = "white")
...
for (family in familys) {
  ...
  # 提取当前目的基因列表
  family_ko_list <- subset(fun_lamda_rf_enrich, Tree_Family == family)$KOID
  family_co_ko_list <- subset(fun_lamda_rf_enrich, Tree_Family == family & padj < 0.05)$KOID
...
ggsave(family_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_familys_fun_coevolved_ko.pdf",
       width = 36, height = 36, units = "cm", dpi = 900, bg = "white")

ggsave(family_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_familys_fun_coevolved_ko.png",
       width = 36, height = 36, units = "cm", dpi = 900, bg = "white")
...
for (class in classs) {
  ...
  # 提取当前目的基因列表
  class_ko_list <- subset(fun_lamda_rf_enrich, Tree_Class == class)$KOID
  class_co_ko_list <- subset(fun_lamda_rf_enrich, Tree_Class == class & padj < 0.05)$KOID
...
ggsave(class_enriched_g,
       filename = "KO_enrichment_each_plant_classs_fun_coevolved_ko.pdf",
       width = 18, height = 8, units = "cm", dpi = 900, bg = "white")

ggsave(class_enriched_g,
       filename = "KO_enrichment_each_plant_classs_fun_coevolved_ko.png",
       width = 18, height = 8, units = "cm", dpi = 900, bg = "white")
