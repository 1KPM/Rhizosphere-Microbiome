...
####(1) select KO
select_KO <- read.csv(file = "./stats/fun_order_coevoled_top4_ko_qa.csv", fileEncoding = "GBK")
...
select_KO_info <- read.csv(file = "./stats/fun_order_coevoled_top4_ko_lambda_rf.csv", fileEncoding = "GBK")
...
### 可视化7个目的代表性KO
# 定义要循环的KO列表
rep_kos <- c("K00263", "K01916", "K07714", "K23139")
...
combined_plot <- wrap_plots(plot_list, ncol = 4) +
  plot_layout(widths = c(1, 1, 1, 1),  # 稍微调整宽度比例
              heights = 1,
              guides = "collect") &
  theme(
