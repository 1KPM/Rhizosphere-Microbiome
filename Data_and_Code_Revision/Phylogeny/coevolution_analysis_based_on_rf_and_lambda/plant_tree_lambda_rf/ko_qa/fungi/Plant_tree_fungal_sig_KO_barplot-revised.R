...
fun_lambda_rf_ko <- read.csv(file = "./stats/TableS7E-fun_KO_qa_log_lamda_indicators.csv", row.names = 1) 
table(fun_lambda_rf_ko$padj < 0.05)
# functional coevolution was limited in fungi (261/5160 = 5.06% of KOs) 

# 虽然显著性检验使用的是绝对丰度，但使用TPM数据展示更好看（避免绝对丰度异常值）
fun_ko_qa <- read.csv(file = "../../../../kegg_abundance/relative/Fungi_core0.2_tpm.csv", row.names = 1, header = T)
...
fun_order_coevoled_top_ko <- fun_order_coevoled_top_ko[,c(2:15,1,16)]
fun_order_coevoled_top_ko <- subset(fun_order_coevoled_top_ko, Tree_Order != "Others")
# 只有4个有发育信号的KO对应的植物科属于同一目 (这是很重要排除假阳性的筛选)
# 保存这个筛选结果
fun_lambda_rf_ko$Coevolved <- NA  # 初始化新列
matching <- fun_lambda_rf_ko$KOID %in% fun_order_coevoled_top_ko$KOID
fun_lambda_rf_ko$Coevolved[matching] <- fun_lambda_rf_ko$Tree_Order[matching]
write.csv(fun_lambda_rf_ko, file = "./stats/TableS7E-revised-fun_KO_qa_log_lamda_indicators.csv") 

# 丰度阈值筛选，可选
fun_ko_qa_mean <- data.frame(KOID = fun_ko_qa$KOID, TPM_mean = rowMeans(fun_ko_qa[,-1]))
fun_order_coevoled_top_ko <- merge.data.frame(fun_ko_qa_mean, fun_order_coevoled_top_ko, by = "KOID")

# 继续筛选就没有了
# fun_order_coevoled_top_ko <- subset(fun_order_coevoled_top_ko, padj < 0.01 & rf_FDR_Order < 0.01)
# 0个KO

# 5. 准备用于热图绘制的KO丰度数据
fun_order_coevoled_top_ko_qa <- merge.data.frame(fun_order_coevoled_top_ko[,c("KOID", "Tree_Order")], fun_ko_qa, by = "KOID")

# write.csv(fun_order_coevoled_top_ko_qa, file = "./stats/fun_order_coevoled_top4_ko_qa.csv")
# write.csv(fun_order_coevoled_top_ko, file = "./stats/fun_order_coevoled_top4_ko_lambda_rf.csv")

common_samples <- intersect(read.tree('../../../../metadata/tree_metadata_merge_info_final_align_tree.nwk')$tip.label,
                            colnames(fun_order_coevoled_top_ko_qa)[!colnames(fun_order_coevoled_top_ko_qa) %in% c("KOID", "Tree_Order")])
ko_tpm_data_filtered <- fun_order_coevoled_top_ko_qa[, c("KOID", "Tree_Order", common_samples)]
...
pwidth_each <- c(1.5, 1.5, 0.05)  #pwidth_each这个参数始终没有搞明白
...
ggsave(plant_final_plot_scaled,
       filename = "./plant_tree_barplot/plant_order_level_coevolved_Fungal_top_ko-Facet-Final-used-revised.pdf",
       width = 8, height = 8, units = "cm", dpi = 900, bg = "white")

ggsave(plant_final_plot_scaled,
       filename = "./plant_tree_barplot/plant_order_level_coevolved_Fungal_top_ko-Facet-Final-used-revised.png",
       width = 8, height = 8, units = "cm", dpi = 900, bg = "white")
