...
# library(devtools)
# source("https://bioconductor.org/biocLite.R"); biocLite(c("RBGL","graph")) # doesnt work
...
names(bac_ko_ra) <- gsub("RS.", "", names(bac_ko_ra))
...
bac_lamda_file <- read.csv("../../../../12.phylogenetic_signal_λ/metagenomic/core_KO/Bacteria_core0.2_qa_log_phylogenetic_signal.csv", row.names = 1) %>% 
  rownames_to_column("KOID")
table(bac_lamda_file$padj < 0.05) ###7326个系统发图显著的KO

bac_rf_file <-  read.csv("../../../../13.random_forest/metagenome/bac_ko_core0.2_qa_log_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
# 该文件只包含显著性矫正P值小于0.2的KO
row.names(bac_rf_file) <- gsub("\\|.*", "", row.names(bac_rf_file))
bac_rf_file$KOID <- row.names(bac_rf_file) 
table(bac_rf_file$P.adj < 0.05) ###3475个随机森林显著的KO 

# 数据合并
bac_group_file <- bac_lamda_file %>%
  left_join(bac_rf_file, by = "KOID") %>%
  left_join(core_annotation, by = "KOID") 

table(bac_group_file$Group)
table(bac_group_file$padj < 0.05)  ###7326 ko
table(bac_group_file$P < 0.05)
table(bac_group_file$P.adj < 0.05) ###3475 ko
...
#####################################################################################################
####统计韦恩图
bac_ko_list <- bac_group_file$KOID
bac_lamda_ko_list <- subset(bac_group_file, padj < 0.05)$KOID
bac_rf_ko_list <- subset(bac_group_file, P.adj < 0.05)$KOID
order_co_ko_list <- subset(bac_group_file, padj < 0.05 & P.adj < 0.05)$KOID #2779


order_venn <- Venn(list('Phylogenetically conserved KOs' = bac_lamda_ko_list, 
                        'Order indicators' = bac_rf_ko_list))

win.metafile("./plots/bac_KO_order_venn.emf")
plot(order_venn, doWeights = TRUE, type="circles", 
     show = list(Faces = T, DarkMatter = FALSE))
dev.off()

###Class和Family水平
class <- read.csv("../../../../13.random_forest/metagenome/bac_ko_qa_log_Class_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
row.names(class) <- gsub("\\|.*", "", row.names(class))
class$KOID <- row.names(class)
family <- read.csv("../../../../13.random_forest/metagenome/bac_ko_qa_log_Family_tree_rf_res_diff_KO.csv", sep = ",", header = T, row.names = 1)
...
win.metafile("./plots/bac_KO_class_venn.emf")
...
win.metafile("./plots/bac_KO_family_venn.emf")
...
bac_lamda_rf_file <- bac_lamda_file %>%
  ...
  left_join(bac_rf_file[bac_rf_file$P.adj < 0.05, c("KOID", "Group", "P.adj")], by = "KOID") %>%  #bac_rf_file就是order水平
  left_join(family[family$P.adj < 0.05, c("KOID", "Group","P.adj")], by = "KOID") 

names(bac_lamda_rf_file)[17:22] <- c("Tree_Class", "rf_FDR_Class", "Tree_Order", "rf_FDR_Order", "Tree_Family", "rf_FDR_Family") 

write.csv(bac_lamda_rf_file, file = "./stats/bac_KO_qa_log_lamda_indicators.csv")
# write.csv(unique(bac_lamda_rf_file[,-c(11:16)]), file = "./stats/TableS7D-bac_KO_qa_log_lamda_indicators.csv")

table(bac_lamda_rf_file[!is.na(bac_lamda_rf_file$Tree_Order) & bac_lamda_rf_file$padj < 0.05,]$Tree_Order)
#########

bac_lamda_rf_file <- read.csv(file = "./stats/bac_KO_qa_log_lamda_indicators.csv")
bac_lamda_rf_file$Superpathway <- bac_lamda_rf_file$level2
table(bac_lamda_rf_file$level2) # superpathway level

ko_superpathway_color <- read.csv('../../../../metadata/superpathway_color.csv')


raw_df <- bac_lamda_rf_file[, c("KOID","lambda", "P", "padj", "Superpathway", "Tree_Class", "Tree_Order", "Tree_Family")] 
###原文件是在KOID上合并的，在superpathway上有重复，如同一个KO多次在一个pathway上出现
raw_df <- unique.data.frame(raw_df) #去重复
raw_df <- raw_df[!is.na(raw_df$Superpathway),] # 8438个

all_df <- data.frame(t(table(raw_df$Superpathway)))
bac_lamda_df <- data.frame(t(table(raw_df[raw_df$padj < 0.05, 'Superpathway'])))
bac_rf_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]), 'Superpathway'])))
co_df_class <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Class"]) & raw_df$padj < 0.05, 'Superpathway'])))
bac_rf_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]), 'Superpathway'])))
co_df_order <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Order"]) & raw_df$padj < 0.05, 'Superpathway'])))
bac_rf_df_family <- data.frame(t(table(raw_df[!is.na(raw_df["Tree_Family"]), 'Superpathway'])))
...
process_and_plot <- function(tax_level, level_order, file_suffix, facet_width_factor) {
  ...
  height <- 8.5
  base_name <- paste0('./plots/Superpathway_count_bacterial_co-evolved-KOs-Number-', file_suffix)     # Number/Percentage
...
bac_lamda_rf_file <- read.csv(file = "./stats/bac_KO_qa_log_lamda_indicators.csv")
names(bac_lamda_rf_file)
bac_lamda_rf_enrich <- unique(bac_lamda_rf_file[,-c(1,11:17)]) ###12447行，12447个KO， 没错的

table(subset(bac_lamda_rf_enrich, padj < 0.05 & Tree_Order != "NA")$Tree_Order)

bac_core_ko_list <- bac_lamda_rf_enrich$KOID
bac_lamda_ko_list <- subset(bac_lamda_rf_enrich, padj < 0.05)$KOID
class_ko_list <- subset(bac_lamda_rf_enrich, Tree_Class != "NA")$KOID
class_co_ko_list <- subset(bac_lamda_rf_enrich, padj < 0.05 & Tree_Class != "NA")$KOID
order_ko_list <- subset(bac_lamda_rf_enrich, Tree_Order != "NA")$KOID
order_co_ko_list <- subset(bac_lamda_rf_enrich, padj < 0.05 & Tree_Order != "NA")$KOID
family_ko_list <- subset(bac_lamda_rf_enrich, Tree_Family != "NA")$KOID
family_co_ko_list <- subset(bac_lamda_rf_enrich, padj < 0.05 & Tree_Family != "NA")$KOID

table(bac_lamda_rf_enrich$Tree_Class)

#进行富集分析
bac_core_ko_result=enrichKEGG(bac_core_ko_list,
                          organism = "ko", #物种选择ko
...
bac_lamda_ko_result=enrichKEGG(bac_lamda_ko_list,
                            organism = "ko", #物种选择ko
                            pvalueCutoff = 0.05,  #p值cutoff
                            pAdjustMethod = "fdr", #fdr矫正p值
                            qvalueCutoff = 0.2   #q值cutoff
...
#保存富集分析结果
write.csv(file="./stats/KO_enrichment_all_bac_core_ko.csv", data.frame(bac_core_ko_result), row.names=F)
write.csv(file="./stats/KO_enrichment_all_bac_lamda_ko.csv", data.frame(bac_lamda_ko_result), row.names=F)
...
# 绘制气泡图
barplot(bac_core_ko_result)
barplot(bac_lamda_ko_result)
barplot(order_ko_result)
barplot(order_co_ko_result)

####整合图
inter_enriched <- rbind.data.frame(data.frame(bac_core_ko_result),
                                   data.frame(bac_lamda_ko_result),
...
inter_enriched$Group <- c(rep("All", dim(data.frame(bac_core_ko_result))[1]), 
                          rep("Phylogenetically conserved", dim(data.frame(bac_lamda_ko_result))[1]),
...
ggsave(inter_enriched_g2,
       filename = "./plots/KO_enrichment_bac_lamda_and_Phylogenetically_conserved_ko.pdf",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
ggsave(inter_enriched_g2,
       filename = "./plots/KO_enrichment_bac_lamda_and_Phylogenetically_conserved_ko.png",
       width = 20, height = 32, units = "cm", dpi = 900, bg = "white")
...
table(bac_lamda_rf_enrich$Tree_Order)

Fabales_ko_list <- subset(bac_lamda_rf_enrich, Tree_Order == "Fabales")$KOID
Fabales_co_ko_list <- subset(bac_lamda_rf_enrich, Tree_Order == "Fabales" & padj < 0.05)$KOID
#进行富集分析
Fabales_ko_result=enrichKEGG(Fabales_ko_list,
                                organism = "ko", #物种选择ko
                                pvalueCutoff = 0.05,  #p值cutoff
                                pAdjustMethod = "fdr", #fdr矫正p值
                                qvalueCutoff = 0.2   #q值cutoff
...
Fabales_co_ko_result=enrichKEGG(Fabales_co_ko_list,
                                organism = "ko", #物种选择ko
                                pvalueCutoff = 0.05,  #p值cutoff
                                pAdjustMethod = "fdr", #fdr矫正p值
                                qvalueCutoff = 0.2   #q值cutoff
...
barplot(Fabales_ko_result)
barplot(Fabales_co_ko_result)

#保存富集分析结果
write.csv(file="./stats/KO_enrichment_Fabales_bac_coevolved_ko.csv", data.frame(Fabales_co_ko_result), row.names=F)
write.csv(file="./stats/KO_enrichment_Fabales_bac_rf_ko.csv", data.frame(Fabales_ko_result), row.names=F)
...
for (order in orders) {
  ...
  # 提取当前目的基因列表
  order_ko_list <- subset(bac_lamda_rf_enrich, Tree_Order == order)$KOID
  order_co_ko_list <- subset(bac_lamda_rf_enrich, Tree_Order == order & padj < 0.05)$KOID
...
ggsave(order_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_orders_bac_coevolved_ko.pdf",
       width = 24, height = 12, units = "cm", dpi = 900, bg = "white")

ggsave(order_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_orders_bac_coevolved_ko.png",
       width = 24, height = 12, units = "cm", dpi = 900, bg = "white")
...
for (family in familys) {
  ...
  # 提取当前目的基因列表
  family_ko_list <- subset(bac_lamda_rf_enrich, Tree_Family == family)$KOID
  family_co_ko_list <- subset(bac_lamda_rf_enrich, Tree_Family == family & padj < 0.05)$KOID
...
ggsave(family_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_familys_bac_coevolved_ko.pdf",
       width = 36, height = 36, units = "cm", dpi = 900, bg = "white")

ggsave(family_enriched_g,
       filename = "./plots/KO_enrichment_each_plant_familys_bac_coevolved_ko.png",
       width = 36, height = 36, units = "cm", dpi = 900, bg = "white")
...
for (class in classs) {
  ...
  # 提取当前目的基因列表
  class_ko_list <- subset(bac_lamda_rf_enrich, Tree_Class == class)$KOID
  class_co_ko_list <- subset(bac_lamda_rf_enrich, Tree_Class == class & padj < 0.05)$KOID
...
ggsave(class_enriched_g,
       filename = "KO_enrichment_each_plant_classs_bac_coevolved_ko.pdf",
       width = 18, height = 8, units = "cm", dpi = 900, bg = "white")

ggsave(class_enriched_g,
       filename = "KO_enrichment_each_plant_classs_bac_coevolved_ko.png",
       width = 18, height = 8, units = "cm", dpi = 900, bg = "white")
