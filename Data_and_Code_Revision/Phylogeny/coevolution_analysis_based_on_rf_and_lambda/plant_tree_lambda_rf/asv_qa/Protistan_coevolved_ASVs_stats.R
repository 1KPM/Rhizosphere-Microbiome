...
# 导入系统发育信号分析结果
lamda_file <- read.csv("../../../12.phylogenetic_signal_λ/amplicon/core_asv/Protists_absolute_format_core_ASV_phylogenetic_signal.csv", row.names = 1) %>%
  rownames_to_column("FeatureID")

# 导入随机森林（Class, Order, Family水平）鉴定的差异ASV结果
rf_class <- read.csv("../../../13.random_forest/amplicon/pro_qa_log_Class_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
rf_order <- read.csv("../../../13.random_forest/amplicon/pro_qa_log_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
rf_family <- read.csv("../../../13.random_forest/amplicon/pro_qa_log_Family_rf_res_diff_ASV.csv", sep = ",", header = T, row.names = 1)
...
write.csv(lambda_rf_file, file = "./stats/pro_core_asv_qa_log_lambda_indicators.csv")
...
pro_order_coevoled_asv <- lambda_rf_file[!is.na(lambda_rf_file$Tree_Order) & lambda_rf_file$padj < 0.05,]
pro_order_coevoled_top_asv <- merge(pro_order_coevoled_asv, unique(metadata_tree[,c("Family","Order")]),
                                    by.x = "Tree_Family", by.y = "Family", all.x = T)
pro_order_coevoled_top_asv <- pro_order_coevoled_top_asv[pro_order_coevoled_top_asv$Tree_Order == pro_order_coevoled_top_asv$Order.y,]
names(pro_order_coevoled_top_asv)
pro_order_coevoled_top_asv <- pro_order_coevoled_top_asv[,c(2:17,1,18)]
pro_order_coevoled_top_asv <- subset(pro_order_coevoled_top_asv, Tree_Order != "Others")
# 23个有发育信号的ASV对应的植物科属于同一目 (这是很重要排除假阳性的筛选)

# 保存这个筛选结果
pro_lambda_rf_asv <- lambda_rf_file
pro_lambda_rf_asv$Coevolved <- NA  # 初始化新列
matching <- pro_lambda_rf_asv$FeatureID %in% pro_order_coevoled_top_asv$FeatureID
pro_lambda_rf_asv$Coevolved[matching] <- pro_lambda_rf_asv$Tree_Order[matching]
write.csv(pro_lambda_rf_asv, file = "./stats/TableS7C-revised-pro_ASV_qa_log_lamda_indicators.csv") 
...
# 79个ASV; 5.73% (145/2532) of Protistsl
...
win.metafile("./plots/Protistan_class_venn.emf")
...
win.metafile("./plots/Protistan_order_venn.emf")
...
win.metafile("./plots/Protistan_family_venn.emf")
plot(family_venn, doWeights = TRUE, type="circles",
     show = list(Faces = T, DarkMatter = FALSE))
dev.off()

### 统计并绘制Class水平的分布条形图 ------------------------------------------
lambda_rf_file <- read.csv(file = "./stats/pro_core_asv_qa_log_lambda_indicators.csv")
top_class_bac <- read.csv('../../../feature_table/Protist_top10Class_color_meanValue.csv')
...
name = './plots/Class distribution co-evolved Protists ASVs'
