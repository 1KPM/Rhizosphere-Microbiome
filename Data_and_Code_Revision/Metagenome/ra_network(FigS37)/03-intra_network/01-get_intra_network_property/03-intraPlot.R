...
### Import data ----------------------------------------------------------------
maptree_edge <- read.csv('intra_edge_mt2.csv')
maptree_node <- read.csv('intra_node_mt2.csv')
maptree_node$Clade <- ifelse(substr(maptree_node[, 1], 1, 1) == "b", "Bacteria", ifelse(substr(maptree_node[, 1], 1, 1) == "f", "Fungi", "Protist"))

# ------------------------------------------------------------------------------

maptree_node$module[maptree_node$module > 7] <- 8
### get results ----------------------------------------------------------------

color_manual <- colorRampPalette(brewer.pal(8, 'Accent'))(8)

p <- ggplot() + 
  geom_segment(aes(x = X1, y = Y1, xend = X2, yend = Y2, color = Correlation), 
               alpha = 0.05, linewidth = 0.1, data = maptree_edge) + 
  geom_point(aes(X1, X2, fill = factor(module), size = Degree ** 0.6), pch = 21, data = maptree_node) +
