...
for (ord in names(top10order_list)) {
    ...
    hub_info$Clade <- ifelse(substr(hub_info[, 1], 1, 1) == "b", "Bacteria", ifelse(substr(hub_info[, 1], 1, 1) == "f", "Fungi", "Protist"))

    names(hub_info)[2:4] <- centrality
    tmp_df <- hub_info %>%
