...
combinations3 <- strsplit(summary_table$Intersections[1:50], "; ")

p1_revision2 <- upset(tmp_data,
                      sets = All_list,
                      keep.order = TRUE,
                      order.by = "freq",
...
png(paste0(name, "_UpSetR.png"), width = width, height = height, units = "cm", res = 600); print(p1_revision2); dev.off()
pdf(paste0(name, "_UpSetR.pdf"), width = width / 2.54, height = height / 2.54); print(p1_revision2); dev.off()
