pwd <- "/public/home/wangxiaolin_lab/group/core0.2ko_network/Fabales_crosskingdom"
setwd(pwd)
...
##输入数据来自单个目总网络
KO_network_cor_all <- read.csv('../Fabales_network/Fabales_KO_WGCNA_spearman_cor_raw.csv', row.names = 1)
KO_network_node_all <- read.csv('../Fabales_network/Fabales_KO_network_node_property.csv')
KO_network_edge_all <- read.csv('../Fabales_network/Fabales_KO_network_edge_property.csv')
KO_subset <- read.csv('../Fabales_network/Fabales_KO_abundance.csv', row.names = 1,check.names = F)
