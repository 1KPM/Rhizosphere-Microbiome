...
### Import data ----------------------------------------------------------------
maptree_edge <- read.csv('edge_mt2.csv')
maptree_node <- read.csv('node_mt2.csv')
maptree_node$Clade <- ifelse(substr(maptree_node[, 1], 1, 1) == "b", "Bacteria", ifelse(substr(maptree_node[, 1], 1, 1) == "f", "Fungi", "Protists"))
