# ******************************************************************************
# @File: 04-robustness_plot_kingdom.R
...
type <- c("all", "intra","inter")
color_manual <- colorRampPalette(RColorBrewer::brewer.pal(8, "Dark2"))(2)
# ------------------------------------------------------------------------------


### Get results ----------------------------------------------------------------
# 1. Core KO 累积去除后的Robustness
for (typ in type) {
  file_path <- paste0("03-robustness/", typ, "_robustness_by_cumulative_ko.csv")
  ...
  name <- paste0(dir_name, "/robustness_by_cumulative_ko_",typ)
  width <- 5.3
  height <- 3.8
...
# 2. Core KO 按Kingdom去除后的Robustness
for (typ in type) {
  ...
  p <- ggplot(
    ...
    scale_y_continuous(breaks = c(0.90, 0.93, 0.96, 0.99)) +   # 自定义刻度
