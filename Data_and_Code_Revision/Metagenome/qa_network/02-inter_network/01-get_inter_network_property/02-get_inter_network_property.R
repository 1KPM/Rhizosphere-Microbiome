...
# Create directory
dir_name <- "02-get_inter_network_property1"
...
### Import data ----------------------------------------------------------------
edge <- read.csv("inter_network_edge.csv", header = T)
node <- read.csv("inter_network_node.csv", header = T)
r_table <- read.csv("inter_r_table.csv", header = T, row.names = 1)
# ------------------------------------------------------------------------------
...
write.csv(maptree_edge,"inter_edge_mt2.csv",row.names = F)

maptree_module <- maptree[[2]]
write.csv(maptree_module, "inter_module_mt2.csv", quote = F, row.names = F)
...
write.csv(maptree_node,file = "inter_node_mt2.csv")
...
ggsave('inter_network_zipi.pdf', p_zipi, width = 8, height = 6)
all_hub_info <- cbind(zp_result_df, all_hub)
write.csv(all_hub_info, 'inter_network_hub_info.csv')