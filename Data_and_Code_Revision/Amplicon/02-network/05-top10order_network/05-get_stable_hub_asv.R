...
# Create directory
dir_name <- "05-get_stable_hub_asv"
...
# 在8个及其8个以上目中都为core ASV的定义为common hub ASV
common_hub_asv <- summary_table$Members[str_count(summary_table$Intersections, ";") >= 7]
common_hub_asv <- unlist(strsplit(common_hub_asv, split = "; "))

name <- paste0(dir_name, "/", net, "_top10order")
write.csv(summary_table, paste0(name, "_upsetR_results.csv"), quote = F, row.names = F)
write.csv(common_hub_asv, paste0(name, "_common_hub_asv.csv"), quote = F, row.names = F)
...
raw_hub_asv <- row.names(raw_hub[raw_hub$roles != "Peripherals",])


# 明确背景全集
background_asv <- core_taxonomy$FeatureID

# 严谨计算 2x2 列联表的四个象限 (Quadrant)
# [1] 既是 raw_hub 又是 common_hub (A ∩ B)
n_both <- length(intersect(raw_hub_asv, common_hub_asv))

# [2] 是 raw_hub，但不是 common_hub (A - B)
n_raw_not_common <- length(setdiff(raw_hub_asv, common_hub_asv))

# [3] 是 common_hub，但不是 raw_hub (B - A)
n_common_not_raw <- length(setdiff(common_hub_asv, raw_hub_asv))

# [4] 既不是 raw_hub，也不是 common_hub (全集 - (A ∪ B))
union_asv <- union(raw_hub_asv, common_hub_asv)
n_neither <- length(setdiff(background_asv, union_asv))
...
# （顺带修复下方绘图代码中未定义的变量名）
data_list <- list()
data_list[['Raw hub']] <- raw_hub_asv 
data_list[['Common hub']] <- common_hub_asv
...
stable_hub_asv <- intersect(raw_hub_asv, common_hub_asv)
write.csv(stable_hub_asv, paste0(dir_name, "/", net, "_stable_hub_asv.csv"), quote = F, row.names = F)
# ------------------------------------------------------------------------------
