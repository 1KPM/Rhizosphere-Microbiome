...
r_table <- read.csv("01-get_all_network_data/WGCNA_spearman_r_table.csv", header = T, row.names = 1)
...
name <- paste0(dir_name, '/', type, '_maptree_')
write.csv(maptree_node, paste0(name, "node.csv"), quote = F, row.names = F)
write.csv(maptree_edge, paste0(name, "edge.csv"), quote = F, row.names = F)
write.csv(maptree_module, paste0(name, "module.csv"), quote = F, row.names = F)
