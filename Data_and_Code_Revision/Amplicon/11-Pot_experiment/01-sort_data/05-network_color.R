...
### Import data ----------------------------------------------------------------
core_table <- read.csv("04-core_table/core_feature_table_absolute.csv", row.names = 1, check.names = F)
core_taxonomy <- read.csv("04-core_table/16S_core_taxonomy.csv", row.names = 1)
# ------------------------------------------------------------------------------
...
get_top_taxa <- function(data, tax_level) {
    data %>%
...
top_5percent_taxa <- list(
    top_16s_phylum = get_top_taxa(top_tax, "Phylum"),
    top_16s_order  = get_top_taxa(top_tax, "Order")
)
